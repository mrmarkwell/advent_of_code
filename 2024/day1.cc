...
are indeed very different. Or are they? The Historians can't agree on which
group made the mistakes or how to read most of the Chief's handwriting, but in
the commotion you notice an interesting detail: a lot of location IDs appear in
both lists! Maybe the other numbers aren't location IDs at all but rather
misinterpreted handwriting. This time, you'll need to figure out exactly how
often each number from the left list appears in the right list. Calculate a
...
int main() {
  std::vector<std::string> lines =
      aoc::LoadStringsFromFileByLine("./2024/day1.txt");
  std::vector<int> l1, l2;
  for (const auto& line : lines) {
    std::pair<int, int> entries = ParseLine(line);
    l1.push_back(entries.first);
    l2.push_back(entries.second);
  }
  ...
  std::cout << "DIFF PART 1: " << sum << std::endl;

  std::unordered_map<int, int> dict;
  sum = 0;
  for (int i = 0; i < l1.size(); i++) {
    if (dict.find(l1[i]) == dict.end()) {
      int count = 0;
  ...
  std::cout << "DIFF PART 2: " << sum << std::endl;
